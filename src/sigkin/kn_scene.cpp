...
void KnScene::connect ( KnSkeleton* s )
{
	...
	SnModel* lsphere = new SnModel;
	lsphere->model()->make_sphere(GsPnt::null, 1, 20, true);

	//SnModel* lcylinder = new SnModel;
	...
	for(int i=3;i<=8;i++)
	{
		...
		lg->visible(true);
		lg->separator(true);
		lg->add(lt = new SnTransform);
		lg->add(lm = new SnMaterial);
		lg->add(lsphere);
		lpos_.push() = &(lt->get());
		lt->get().translation(joints[i]->get_gcoord());
		lm->material().diffuse = GsColor::green;
		add(limbs_[i-3]);
	}
	...
	{
		lg = new SnGroup;
		GsPnt a (lpos_[i]->e14, lpos_[i]->e24, lpos_[i]->e34);

		GsPnt b (lpos_[i-1]->e14, lpos_[i-1]->e24, lpos_[i-1]->e34);
		gsout<<"Adding cylinder between "<<a<<" and "<<b<<gsnl;
		lcylinder->model()->make_cylinder(a,b,1,1,20,true);
		limb_links_[i-1] = lg;
		lg->visible(true);
		lg->separator(true);
		lg->add(lm = new SnMaterial);
...
void KnScene::update ( int j )
{
	...
	//_jgroup[j]->visible(false);
	if(j >=3 && j<=8)
	{
		if(fabrik_solver_)
		{
			_jgroup[j]->visible(false);
                	limbs_[j-3]->visible(true);
                	lpos_[j-3]->setrans(joint->get_gcoord());
			/*if(j == 4 || j == 5)
			{
				int ll = j-4;
				limb_links_[ll]->init();
				GsPnt a (lpos_[ll]->e14, lpos_[ll]->e24, lpos_[ll]->e34);
                		GsPnt b (lpos_[ll+1]->e14, lpos_[ll+1]->e24, lpos_[ll+1]->e34);
                		gsout<<"Adding cylinder "<<j<<" between "<<a<<" and "<<b<<gsnl;
                		lcylinder->model()->make_cylinder(a,b,1,1,20,true);
                		limb_links_[ll]->visible(true);
                		limb_links_[ll]->separator(true);
                		limb_links_[ll]->add(lm = new SnMaterial);
                		limb_links_[ll]->add(lcylinder);
                		lm->material().diffuse = GsColor::green;
			}
			else if(j == 7 || j == 8)
			{
		...
		{
			_jgroup[j]->visible(true);
			limbs_[j-3]->visible(false);
		}
	}
}
