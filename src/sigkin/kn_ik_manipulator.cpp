...
void KnIkManipulator::solve_method ( SolveMethod sm, KnScene* knscene, KnColdet* coldet )
{
	...
	// finally:
	_method = sm;
	if( _method == Fabrik && _knscene )
		_knscene->setFabrikSolver();
	_res = KnIk::Ok;
...
bool KnIkManipulator::initJointGCoord()
{
        GsMat goal = _manip->mat();
        _ik->set_local( goal );
        _ik->initJointGlobalCoord(goal,_iksearch.oangle);
        return(_ik->checkJointInit());
}
...
KnIk::Result KnIkManipulator::iksolve ()
{
	...
	else if ( _method==SearchOrbit )
	{
		GsMat goal = _manip->mat();
		_ik->set_local ( goal );
		double init_time = gs_time();
		_res = _ik->solve ( goal, _iksearch, _coldet );
		double end_time = gs_time();
		double comp_time = end_time - init_time;
		gsout << "Current time: " << gs_time() << gsnl;
		gsout << "Time taken to compute: " << comp_time << gsnl;
		if ( _res==KnIk::Ok )
		{	_ik->apply_last_result();
			if ( _knscene ) _knscene->update();
			if ( _knscene->skeleton()->skin() ) _knscene->skeleton()->skin()->update();
		}
		double er_time = gs_time();
		double rend_time = er_time - init_time;
		gsout << "Time taken to render: " << rend_time << gsnl;

	}
	else if ( _method==Fabrik )
	{
		GsMat goal = _manip->mat();
		_ik->set_local ( goal );
		double init_time = gs_time();
		...
		double comp_time = end_time - init_time;
		gsout << "Current time: " <<gs_time() << gsnl;
		gsout << "Time taken to compute FABRIK: " << comp_time << gsnl;
		if (_knscene ) _knscene->update();
		double er_time = gs_time();
		double rend_time = er_time - init_time;
		gsout << "Time taken to render FABRIK: " << rend_time << gsnl;
	}
	else if ( _method==IkBody )
	{	
		float init_time = gs_time();
		_res = _ikbody->solve ( _ikbodyid, _manip->mat() );
		float comp_time = gs_time() - init_time;
		gsout << "Time taken to compute: " << comp_time << gsnl;
		if ( _knscene ) _knscene->update();
		if ( _knscene->skeleton()->skin() ) _knscene->skeleton()->skin()->update();
		float rend_time = gs_time() - init_time;
		gsout << "Time taken to render: " << rend_time << gsnl;
	}

	if (_postcbfunc) 
		(_postcbfunc) ( this, _postudata );
