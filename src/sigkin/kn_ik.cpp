...
void KnIk::initJointGlobalCoord( const GsMat& goal, float oang )
{
	/* Notation:
	 * m = mid point in base frame
	 * e = end point in base frame
	 * b = base point in base frame
	 * d1 = distance from the base to the mid joint
	 * d2 = distance from the mid joint to the goal point
	 * dist = distance from the base to the goal point
	 */
	...
	float dist = e.len();
	float distdist = dist*dist;
	//if( dist == 0 ) return NotReachable;
	//
	
	float k = ( d1d1 + d2d2 - distdist ) / (2.0f *d2 *d1);
	if( k <= -1 )
		k = -1;
	float midr = gspi - acosf(k);
	...
	GsPnt m = c;
	r = d1 * GS_ABS(sina);
	m += r * (u * cosf(oang) + v*sinf(oang));

	GsMat bframe;
	base_frame(bframe);
	GsPnt basep (bframe.e14, bframe.e24, bframe.e34);
	GsPnt midp = bframe*m;
	GsPnt endp = bframe*e;

	_base->set_gcoord(basep);
	_mid->set_gcoord(midp);
	_end->set_gcoord(endp);
}

bool KnIk::checkJointInit()
{
	if( _base->isGCoordSet() && _mid->isGCoordSet() && _end->isGCoordSet() )
	{
		gsout<<"Limbs init global coordinates:"<<gsnl;
		gsout<<"=============================="<<gsnl;
		_base->printGCoord();
		_mid->printGCoord();
		_end->printGCoord();
...
KnIk::Result KnIk::fabrik( const GsMat& goal )
{
	GsPnt e ( goal.e14, goal.e24, goal.e34 );
	GsMat bframe;
	base_frame(bframe);
	...
	if(dist > (d1+d2))
	{
		...
		l = _d1/r;
		temp = (1 - l)*p1 + l*e;
		_mid->set_gcoord(temp);
		 
		p1 = _mid->get_gcoord();
		r = ::dist(p1, e);
                l = _d1/r;
                temp = (1 - l)*p1 + l*e;
                _end->set_gcoord(temp);
	}
	else
	{
		int count = 0;
		float diff;
		diff = ::dist(_end->get_gcoord(), e);
	...
	if( _snlines )
	{
		...
		lines.init();
		lines.push ( GsColor::cyan );
                lines.push ( _base->get_gcoord(), _mid->get_gcoord() );
		lines.push ( GsColor::cyan );
                lines.push ( _mid->get_gcoord(), _end->get_gcoord() );
	}
	return KnIk::Ok;
}

void KnIk::fabrik_backward(GsPnt e)
{
	_end->set_gcoord(e);
	float r;
	float l;
	
	r = ::dist(_mid->get_gcoord(),_end->get_gcoord());
	l = _d2/r;

	GsPnt temp;
	temp = (1 - l)*_end->get_gcoord() + l*_mid->get_gcoord();
	_mid->set_gcoord(temp);

	r = ::dist(_base->get_gcoord(),_mid->get_gcoord());
	l = _d1/r;

	temp = (1 - l)*_mid->get_gcoord() + l*_base->get_gcoord();
	_base->set_gcoord(temp);
}

void KnIk::fabrik_forward()
{
	GsMat bframe;
	base_frame( bframe );
	GsPnt b (bframe.e14, bframe.e24, bframe.e34);
	_base->set_gcoord(b);
	...
	r = ::dist(_base->get_gcoord(),_mid->get_gcoord());
	l = _d1/r;
	temp = (1 - l)*_base->get_gcoord() + l*_mid->get_gcoord();
	_mid->set_gcoord(temp);

	r = ::dist(_mid->get_gcoord(),_end->get_gcoord());
	l = _d2/r;
	temp = (1 - l)*_mid->get_gcoord() + l*_end->get_gcoord();
	_end->set_gcoord(temp);
}
