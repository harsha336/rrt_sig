...
void glfwMainLoop(GLFWwindow* gwin);
...
# endif
	...
	//f = std::async(std::launch::async, glfwMainLoop, sw->gwin);
	std::thread t(glfwMainLoop, sw->gwin);
	t.detach();
...
{
	//GS_TRACE1("Called redraw");
	SwSysWin* sw = (SwSysWin*)win;
	WsWindow* swin = sw->swin;
	if(sw->redrawcalled) return;
	sw->redrawcalled = 1;
...
static void key_cb ( GLFWwindow* gwin, int key, int scancode, int action, int mods )
{
	...
	if(action == GLFW_PRESS)
	{
		switch(key)
		{
			...
			case GLFW_KEY_ESCAPE:
			       glfwSetWindowShouldClose(gwin, 1);
		       	       break;
			default:
			       break;	       
		}
...
void glfwMainLoop(GLFWwindow* gwin)
{
	...
	{
		glfwTerminate();
		GS_TRACE1("Window not available. Exiting...");
		exit(1);
	}
	...
	while(!glfwWindowShouldClose(gwin))
	{
		if(sw->needsinit)
			continue;
		win_m.lock();
		//if(sw->redrawcalled || sw->needsinit)
		//{
			sysdraw(swin);
			sw->redrawcalled = 0;
...
static void draw_cb ( GLFWwindow* gwin )
{
	...
	std::lock_guard<std::mutex> lg(win_m);
...
void mouse_cb ( GLFWwindow* gwin, int but, int action, int modifs )
{
	...
	e.button = but+1;
	int lbut, rbut, mbut;
	lbut = glfwGetMouseButton(gwin, GLFW_MOUSE_BUTTON_LEFT);
	rbut = glfwGetMouseButton(gwin, GLFW_MOUSE_BUTTON_RIGHT);
	mbut = glfwGetMouseButton(gwin, GLFW_MOUSE_BUTTON_MIDDLE);
	if(lbut == GLFW_PRESS)
	{
		e.button1 = 1;
	}
	...
	int lshift, rshift, lctrl, rctrl, lalt, ralt;
        lshift = glfwGetKey(gwin, GLFW_KEY_LEFT_SHIFT);
        rshift = glfwGetKey(gwin, GLFW_KEY_RIGHT_SHIFT);
        lctrl = glfwGetKey(gwin, GLFW_KEY_LEFT_CONTROL);
        rctrl = glfwGetKey(gwin, GLFW_KEY_RIGHT_CONTROL);
        lalt = glfwGetKey(gwin, GLFW_KEY_LEFT_ALT);
        ralt = glfwGetKey(gwin, GLFW_KEY_RIGHT_ALT);

        if(lshift == GLFW_PRESS || rshift == GLFW_PRESS)
                e.shift = 1;

        if(lctrl == GLFW_PRESS || rctrl == GLFW_PRESS)
                e.ctrl = 1;
...
void mousepos_cb ( GLFWwindow* gwin, double x, double y )
{
	...
	GsEvent e;
	int lstate, rstate;
	lstate = glfwGetMouseButton(gwin, GLFW_MOUSE_BUTTON_LEFT);
	rstate = glfwGetMouseButton(gwin, GLFW_MOUSE_BUTTON_RIGHT);
	...
	int lbut, rbut, mbut;
        ...
        if(lbut == GLFW_PRESS)
	{
		e.button = GLFW_MOUSE_BUTTON_LEFT+1;
                e.button1 = 1;
	}
        if(rbut == GLFW_PRESS)
	{
		e.button = GLFW_MOUSE_BUTTON_RIGHT+1;
                e.button3 = 1;
	}
        if(mbut == GLFW_PRESS)
	{
		e.button = GLFW_MOUSE_BUTTON_MIDDLE+1;
                e.button2 = 1;
	}
...
void scroll_cb(GLFWwindow* gwin, double xoffset, double yoffset)
{
	GS_TRACE1("Got scroll events");
	SwSysWin* sw = (SwSysWin*)glfwGetWindowUserPointer(gwin);
        WsWindow* swin = sw->swin;
        GsEvent e;
	e.type = GsEvent::Wheel;
	e.wheelclicks = yoffset;
	sysevent(swin, e);
}
...
static void _init_callbacks ( GLFWwindow* gwin )
{
	...
	glfwSetScrollCallback ( gwin, scroll_cb );
