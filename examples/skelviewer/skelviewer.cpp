...
{ 
	...
	{	rootg()->add( _mg=new SnGroup );
		...
		KnIkManipulator* ikm[s];
		for ( int i=0; i<s; i++ )
		{	p->add ( new UiButton ( _effs[i]->name(), EvEff+i ) );
			ikm[i] = new KnIkManipulator;
			ikm[i]->init ( _effs[i]->ik() );
			ikm[i]->solve_method ( KnIkManipulator::Fabrik, _sksc );
			ikm[i]->post_callback ( ManipPCB, this );
			ikm[i]->initJointGCoord();
			ikm[i]->lines ( true );
			_mg->add ( ikm[i] );
		}
	}
