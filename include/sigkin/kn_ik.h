...
{  public :
   ...
   public :
	...
	Result fabrik( const GsMat& goal );
	void fabrik_backward( GsPnt e );
	void fabrik_forward();

	void initJointGlobalCoord( const GsMat& goal, float oang );
	bool checkJointInit();
...
{	float init;		//<! the initial orbit angle taken as starting point for the search
	...
	void print_values()
	{
		gsout<<"KnIkOrbitSearch: init: "<<init<<gsnl;
		gsout<<"KnIkOrbitSearch: inc: "<<inc<<gsnl;
		gsout<<"KnIkOrbitSearch: rate: "<<rate<<gsnl;
		gsout<<"KnIkOrbitSearch: min: "<<min<<gsnl;
		gsout<<"KnIkOrbitSearch: max: "<<max<<gsnl;
		gsout<<"KnIkOrbitSearch: oangle: "<<oangle<<gsnl;
		gsout<<"KnIkOrbitSearch: iterations: "<<iterations<<gsnl;
	}
};
